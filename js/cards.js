
setTimer(
    new Date("May 21, 2022 18:17:04"),
    document.getElementById("card__time1")
   );
  setTimer(
    new Date("July 29, 2022 15:37:25"),
    document.getElementById("card__time2")
   );
   setTimer(
    new Date("Septamber 25, 2022 05:30:55"),
    document.getElementById("card__time3")
   );
   setTimer(
    new Date("June 23, 2022 22:19:45"),
    document.getElementById("card__time4")
   );
   setTimer(
    new Date("May 28, 2022 18:17:04"),
    document.getElementById("card__time5")
   );
  setTimer(
    new Date("July 1, 2022 15:37:25"),
    document.getElementById("card__time6")
   );
   setTimer(
    new Date("Septamber 4, 2022 05:30:55"),
    document.getElementById("card__time7")
   );
   setTimer(
    new Date("June 1, 2022 22:19:45"),
    document.getElementById("card__time8")
   );